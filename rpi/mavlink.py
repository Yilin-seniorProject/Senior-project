...
ret = send_message(master, mavutil.mavlink.MAVLINK_MSG_ID_GLOBAL_POSITION_INT, 1e6)
print("mavlink on ready")

def get_gps_info():
    # Receive message
    try:
        gps_info = master.recv_match(type="GLOBAL_POSITION_INT", blocking=True).to_dict()
        print(f'lat:{gps_info["lat"]}, lon:{gps_info["lon"]}')
        return (gps_info["lat"], gps_info["lon"])
        # print(gps_info)
